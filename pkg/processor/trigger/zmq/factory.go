...
func (f *factory) Create(parentLogger logger.Logger,
	...
	zmqLogger := parentLogger.GetChild("zmq")
	...
	workerAllocator, err := f.GetWorkerAllocator(triggerConfiguration.WorkerAllocatorName,
		...
		func() (worker.Allocator, error) {
			return worker.WorkerFactorySingleton.CreateFixedPoolWorkerAllocator(zmqLogger,
	...
	zmqTrigger, err := newTrigger(zmqLogger,
		workerAllocator,
		configuration,
	)
	if err != nil {
		return nil, errors.Wrap(err, "Failed to create zmq trigger")
	}

	return zmqTrigger, nil
}

// register factory
func init() {
	trigger.RegistrySingleton.Register("zmq", &factory{})
}
